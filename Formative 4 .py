...
def bin_to_dec(binary_string):
    ...
    for i in range(len(binary_string)):
        number += int(binary_string[i])*(2**exponent)
        exponent -= 1
    return number
...
def telephone_cipher(message):
    ...
    telephone = encoder_dict[message[0]]
    cleaned_telephone = ""
    message = message.upper()
    
    for i in range(1, len(message)):
        letter = message[i]
        number = encoder_dict[letter]
          
        if encoder_dict[letter][-1] == telephone[-1][0]:
            telephone += "_"
           
        telephone += encoder_dict[letter]
        cleaned_telephone = "".join(telephone)
...
def telephone_decipher(telephone_string):
    ...
    telephone_list = telephone_string.split("_")
    corrected_telephone_list = []
    current_string = ""

    for i in range(len(telephone_list)):
        for j in range(len(telephone_list[i])): 
            if telephone_list[i][j] == telephone_list[i][j-1] or j == 0:
                current_string += telephone_list[i][j]
            else:
                corrected_telephone_list.append(current_string)
                current_string = telephone_list[i][j]
        corrected_telephone_list.append(current_string)
        current_string = ""

    letter_equivalent = ""
    for k in range(len(corrected_telephone_list)):
        letter = decipher_dict[corrected_telephone_list[k]]
        letter_equivalent += letter
    return letter_equivalent
